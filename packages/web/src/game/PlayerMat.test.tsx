 ...
 * - moweyds 无高清图 → 回退旧布局（legacy，无 mat-board）；
...
describe('<PlayerMat> 族板整图渲染契约', () => {
  ...
  it('moweyds 无高清图 → 回退旧布局', () => {
    const state = fixture(['moweyds', 'terrans'], true);
    const { queryByTestId, getByTestId, container } = render(<PlayerMat state={state} playerIdx={0} />);
    expect(queryByTestId('mat-board-0')).toBeNull();
    // legacy：文字行 power + supply
    expect(getByTestId('power-0')).toBeInTheDocument();
    expect(container.querySelectorAll('.supply-item').length).toBeGreaterThan(0);
    // 另一名玩家（terrans）仍是整图布局
    const { getByTestId: g2 } = render(<PlayerMat state={state} playerIdx={1} />);
    expect(g2('mat-board-1')).toBeInTheDocument();
  });
